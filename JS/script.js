...
function calcularValorFinal() {
    ...
    if (isNaN(preco) || preco <= 0 || pagamento === "") {
        resultado.innerHTML = '<p class="acrescimo">Por favor, preencha todos os campos corretamente.</p>';
        return;
    }
    ...
    resultado.innerHTML = `<h4>Resultado:</h4>
    <p class="${classe}">${mensagem}</p>
    <p>Valor final da compra: <b>R$ ${valorFinal.toFixed(2).replace('.', ',')}</b></p>
    <hr>
    `;
